...
export const multerStorageConfig = multer.diskStorage({
  destination: (req, _file, callback) => {
    ...
    if (fs.readdirSync(uploadPath).length >= 50)
      return callback(
        new Error('Upload limit reached. Please delete some files before uploading new ones.'),
        uploadPath,
      );
...
export const allowedMimeTypes = ['image/jpeg', 'image/png', 'image/webp', 'audio/webm'];
