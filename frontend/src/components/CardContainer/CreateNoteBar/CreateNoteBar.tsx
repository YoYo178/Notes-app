...
export const CreateNoteBar: FC<CreateNoteBarProps> = ({ isVisible }) => {
  ...
  const handleToggleRecording = async () => {
    if (!hasMicPermissions && !isMicDeniedPopupVisible) setIsMicDeniedPopupVisible(true);
    ...
    if (hasSpeechRecognitionSupport && !isTranscribing && !!startTranscribing) startTranscribing();
  };

  return (
    <>
      <CreateNoteModal isOpen={modalOpen} onClose={handleModalClose} noteType={noteType} />
      ...
      {isMicDeniedPopupVisible && (
        <div className='mic-permission-denied-popup-backdrop'>
          <div className='mic-permission-denied-popup'>
            ...
            <p className='mic-permission-denied-description'>
              Please allow microphone access to record audio notes.
            </p>
            <button
              className='mic-permission-denied-dismiss-btn'
              onClick={() => setIsMicDeniedPopupVisible(false)}
            >
              Dismiss
            </button>
          </div>
