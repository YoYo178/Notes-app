...
function profileModalOnSubmit(
  ...
  if (!currentAuthFields) return true;
...
function deleteAccButtonOnClick(deleteUserMutation: TMutation<unknown>) {
  const userAgrees = window.confirm(
    'Are you sure you want to delete your account? All the notes associated with this account will be deleted!',
  );

  if (userAgrees) deleteUserMutation.mutate({});
}
