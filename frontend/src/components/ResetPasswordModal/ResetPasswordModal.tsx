...
function mutationCallbackHandler(
  ...
  if (mutation.isSuccess && mutation.data) {
    setCurrentStage((prev) => {
      if (prev === ResetAccountStages.REDIRECT) return prev;
...
export const ResetPasswordModal: FC = () => {
  const [currentStage, setCurrentStage] = useState<ResetAccountStages>(
    ResetAccountStages.FIND_ACCOUNT,
  );
  ...
  return (
    ...
    >
      ...
      <div className='fpm-error' style={{ display: errorMessage.length ? 'block' : 'none' }}>
        {errorMessage}
      </div>
      <div className='fpm-info' style={{ display: successMessage.length ? 'block' : 'none' }}>
        {successMessage}
      </div>
